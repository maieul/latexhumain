\chapter{Commencer avec Xe\LaTeX\label{commencer}}
...
Il s'agit, comme vous auriez pu le deviner, d'appel à des packages. Un package est un ensemble de fichiers qui ajoutent des fonctionnalités à \LaTeX, c'est l'équivalent d'un plugin sous \logiciel{Firefox}. 

Le premier package est \package{fontspec}. Il est utile à Xe\LaTeX  pour une typographie avancée, notamment pour les langues autres que l'anglais\footnote{Pour être précis, il détermine des commandes utilisées par le package \package{polyglossia} lorsque définis le fran\c cais comme langue principale du document.}. 
...
Enfin, rien de ce qui se trouve après \verb|\end{document}| ne sera analysé par le compilateur. Vous pouvez donc y mettre ce que vous voudrez, cependant nous ne vous le conseillons pas.
