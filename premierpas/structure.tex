\chapter{Commencer avec \logiciel{\LaTeX}}
...
Le premier \concept{package} est \package{fontspec}. Il est utile à XeLaTex pour une typographie avancée, notamment pour les langues autres que l'anglais\footnote{Pour être précis, il détermine des commandes utilisées par le \concept{package} \package{polyglossia} lorsque définis le fran\c cais comme langue principale du document.}. 

Le second \concept{package} est \package{xunicode}. Il permet de gérer \concept[unicode]{l'unicode}, autrement appelé \concept{utf-8}\footnote{En réalité \concept{utf-8} n'est pas tout à fait \concept{unicode}, mais une implémentation de ce dernier. Toutefois, pour simplifier, nous assimilerons les deux.}. Il nous permettra d'avoir des caractères non-latin\renvoi{utf-8}.

\begin{attention}
Attention : ces deux \concept{packages} doivent être appelés dans cet ordre, et non en sens inverse.
\end{attention}
...
C'est deux \notion{packages} sont propres à \logiciel{XeLaTex} : ils ne fonctionneront pas avec un \notion{compilateur} \logicile{\LaTeX}.
...
Tout ce que nous avons vu jusqu'à maintenant faisaient parti de ce qu'on appel \concept[en-tête]{l'en-tête}\footnote{En anglais \concept{header}.} du document. Ce sont des informations qui n'apparaîtront pas dans le document final, mais qui sont utiles à sa composition, autrement dit des méta-données. Tout les \concept{packages} que vous voudrez utiliser seront à appeler dans \concept[en-tête]{l'en-tête}.
