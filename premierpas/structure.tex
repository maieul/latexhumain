...
\begin{anedocte}
Le lecture alerte se demandera sans doute pourquoi il est nécéssaire de mettre \verb|toc| comme premier argument. Cela correspond à l'extension du fichier qui contiendra la table des matières : nous renvoyons vers le chapitre dédiée à ce sujet.\renvoi{toc}
\end{anedocte}
...
On indique le chemin du fichier en séparant chaque dossiers par \verb|\|. \revision{Est-ce le cas sous Windows ?} Par exemple, je souhaite inclure le fichier nommé \verb|c.tex| situé dans le dossier \verb|b|, lui même situé dans le dossier \verb|a|, qui se trouve à côoté du fichier \enquote{père}.

Il faut que je mette dans mon fichier \enquote{père} :
