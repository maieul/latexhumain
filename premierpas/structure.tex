...
\begin{itemize}
\item Le niveau \verb|\chapter| n'existe pas dans la classe \classe{article} ;
...
\verb|\addcontentsline{toc}{1}{2}| où : \label{addcontentsline}
\begin{description}
...
\verb|\input{a/b/c}| ou bien \verb|\include{a/b/c}|.
...
\section{La classe \classe{book} : structuration globale du document}\label{sectionbook}
...
Chacune de ces parties est indiquée par une commande initiale, respectivement : \commande{frontmatter} ; \commande{mainmatter} ; \commande{appendix}\footnote{Cette commande existe aussi dans la classe \classe{article}.}\commande{backmatter}.
...
\item\commande{frontmatter} donne des titres non numérotés mais présents dans la table des matières. En outre les numéros de pages sont en chiffres romains minuscules. 
\item\commande{maintmatter} donne des titres numérotés. La numérotation des pages est réinitialisée et est en chiffre arabe.
\item\commande{appendix} affiche les numéros de titres sous forme de lettres majuscules. Le texte \forme{chapitre} est remplacé par \forme{appendice}.
\item\commande{backmatter} supprime les numéros de chapitre mais les présente dans la table des matières.
\end{enumerate}
