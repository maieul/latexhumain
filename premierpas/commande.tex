...
Si \LaTeX prévoit bien en standard un mécanisme pour permettre cela, celui-ci est  peu souple, puisqu'il ne permet de rendre optionnel que le premier argument, et non les suivants.

Nous conseillons donc de se tourner vers les packages \package{xargs} et \package{ifthenelse}\footcites{xargs}{ifthen}. Nous fournissons dans la suite du présent ouvrage un exemple de commande fonctionnement avec ces packages\renvoi{commandevariante}.
\end{plusloins}
