...
Avec ce package, pour numéroter les vers cités, il faut indiquer \cs{poemlines}\marg{nbr} :  \arg{nbr} 
...
\begin{anedocte}
Dans la typographie française, il est d'usage de mettre un crochet droit [ au début d'un rejet. Ni l'environnement ni le package \package{verse} ne suivent cette règle. Pour obtenir le crochet droit, il faut indiquer dans le préambule \cs{usepackage}\oarg{squarebr}\marg{gmverse}| et indiquer, une fois commencé l'environnement \enviro{verse}, \cs{versehangrightsquare}.
\end{anedocte}

Le package \package{verse} permet aussi d'indenter de façon très souple une strophe. On indique avec la commande \cs{indentpattern}\oarg{$n_1 n_2 n_x$} l'indentation de chaque vers contenu dans la strophe encadrée dans l'environnement \enviro{patverse}: $n_1$ correspond au premier vers, $n_2$ au deuxième et ainsi de suite. Le premier vers n'est jamais indenté, mais il suffit de le faire précéder de \cs{vin}. Avec le code suivant :
...
\end{patverse}  \end{verse} 
 ...
 Si l'on a besoin de répéter une indentation tout le long d'un poème, on remplace \enviro{patverse} par \enviro{patverse*}. Pour citer un long extrait en hexamètre dactylique, plutôt que d'indiquer l'indentation pour chaque vers, il suffit donc d'indiquer \cs{indentpattern}\verb|{01}| et d'insérer tout le poème  dans \enviro{patverse*} pour avoir un vers sur deux indenté.
 
 
 Le package \package{verse} offre bien d'autres possibilités qui dépassent la simple citation de poésie dans le cours d'un texte. A l'instar d'un autre package, \package{poemscol}, c'est un véritable outil pour éditer de la poésie (nous vous renvoyons ici aux manuels de ces packages). Ni l'un ni l'autre ne permettent en revanche de faire des éditions bilingues: pour cela, il faut utiliser les packages \package{ledmac} et \package{ledpar}. \renvoi{ledmac}
