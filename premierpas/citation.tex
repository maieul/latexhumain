...
Le package \package{verse} permet aussi d'indenter de façon très souple une strophe. On indique avec la commande \csp{indentpattern}\oarg{$n_1 n_2 n_x$} l'indentation de chaque vers contenu dans la strophe encadrée dans l'environnement \enviro{patverse}: \meta{$n_1$} correspond au premier vers, \meta{$n_2$} au deuxième et ainsi de suite. Le premier vers n'est jamais indenté, mais il suffit de le faire précéder de \csp{vin}. Avec le code suivant :
...
La commande \csp{textelp}\arg{texte} signale un texte tronqué. L'argument \meta{texte} est inséré après la troncature, entre crochets (par défaut) ; pour ne rien insérer, le laisser vide.
