...
Les images insérables avec \XeLaTeX sont au format JPEG (extension~\ext{jpg}), PNG (extension~\ext{png}) ou PDF (extension~\ext{jpg}). Il est nécessaire d'utiliser le package \package{graphicx}.
...
Notre exemple est volontairement simple. Nous disposons de six familles de manuscrits d'un même texte. La  famille A a engendré les familles B, C, D. Les familles E et F pour leur part sont issues de la famille D. 
...
On peut tracer des filets de séparation des cellules : pour un filet  horizontal, il suffit d'utiliser la commande \csp{hline}. Pour un filet  vertical, il faut mettre le caractère \verb+|+\footnote{Ce caractère, appelé \forme{pipe} ne doit pas être confondu avec un \verb+l+.} entre les types de cellules.
...
Le format \ext{csv} est un format standard de données tabulaires. Malheureusement il ne dispose ni de mise en forme ni de possibilité de fusion de cellules.

\begin{plusloins}Une solution serait peut-être  d'utiliser la macro Calc2LaTeX pour le logiciel OpenOffice.org, mais nous n'avons l'avons pas testée.
\end{plusloins}



Toutefois pour des données simples, comme par exemple un tableau statistiques, il suffit amplement. 
Il vous faut donc exporter votre tableau au format CSV, en choisissant la virgule comme séparateur de champs. La première ligne correspond à l'en-tête. Certains logiciels d'export au format CSV ne laissent pas le choix quant aux séparateurs de champs. Dans ce cas, il faut utiliser la commande \csp{setcsvseparator}\marg{separateur} pour définir les champs. Ainsi si le séparateur est un point-virgule : 
...
Le second se crée avec la commande
\csp{CSVtolongtable}\marg{fichier}\marg{type col.}\marg{premiere}\marg{milieu}\marg{dernière}.
...
\item[\arg{type col.}] indique les alignements de colonnes, comme vus précédemment\renvoi{typecolonnes}.
\item[\arg{premiere}] indique le contenu de la première ligne du  tableau que vous allez générer.
\item[\arg{milieu}] indique le contenu des lignes du milieu.
\item[\arg{dernière}] indique le contenu de la dernière ligne.
\end{description}
...
\item Le package \package{datatool} propose un certain nombre de graphiques statistiques. Il faut enregistrer les données selon une forme liée au package. Nous renvoyons à sa documentation\footnote{datatool}.
\end{itemize}
\end{plusloins}
...
Nous avons un exemple de flottant   dans ce chapitre : la figure \ref{figure:genealogie}\renvoi{figure:genealogie}.
\begin{attention}
...
Pour personnaliser les styles des légendes, la méthode la plus simple reste encore d'utiliser le package \package{caption}. Nous renvoyons à la documentation\footcite{caption}.
...
Pour avoir des flottants orientés dans un autre sens que celui de la page, on utilisera pour ce faire le package \package{rotfloat}\footcite{rotfloat}.

Il est possible d'insérer au sein d'une figure des sous-figures, chacune d'entre elles disposant d'une légende, en plus de la légende principal; il faut utiliser le  \package{subcaption}\footcite{subcaption}.
\end{plusloins}
