...
Pour ce faire, il suffit d'utiliser le package \package{shorttoc}. Celui-ci définit une commande \commande{shorttoc} à la syntaxe simple :
...
Pour afficher les mini-sommaires, il suffit d'utiliser la commande \commande{minitoc} aux endroits voulu, par exemple après chaque titre de chapitre.
...
\begin{attention}
Le package ne peut calculer correctement les sommaires que si la commande \commande{tableofcontents} est utilisée. Si vous souhaitez n'afficher que des mini-sommaires mais pas de tables générales des matières --- ce qui est une idée assez étrange --- vous pouvez utiliser à la place la commande \commande{faketableofcontents}.
\end{attention}

Pour afficher des mini-sommaires par partie, il faut utiliser la commande \commande{parttoc}. Par exemple :
...
Le package \package{minitoc} est très complexe : il permet par exemple choisir aisément les styles du sommaire. Nous renvoyons au manuel pour plus de détail\footcites{minitoc}[en particulier][]{minitoc_typesseting}.
...
Ce package à des nombreuses fonctionnalités : par exemples nous avons vu qu'il proposait des renvois vers les titres des sections.\renvoi{envoititre}
...
Pour ce faire, nous allons utiliser la commande \commande{hypersetup} dans le préambule. La commande prend comme argument des paramètres du package, séparées par une virgule.
