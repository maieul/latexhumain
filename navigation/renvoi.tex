...
Pour renvoyer au numéro de page correspondant à l'étiquette \arg{etiquette} il suffit d'utiliser la commande \csp{pageref}\marg{etiquette}.
...
Pour renvoyer à un numéro de section, il suffit d'utiliser la commande \csp{ref}.
...
Pour renvoyer à un titre de section, il suffit d'utiliser la commande \csp{nameref}.
...
\section{Où placer la commande \cs{label} ?}
...
\begin{itemize}
\item Si l'étiquette \cs{label} est placée \emph{dans}  une commande  d'élément numéroté, elle renvoie à cette élément. Par exemple, pour renvoyer à une figure \renvoi{legende} :
...
Vous êtes bien sûr libre de trouver votre propre système de dénomination des étiquettes. Toutefois il est conseillé d'avoir quelque chose de la forme : \begin{english}\meta{contenu}:\meta{nom}|\end{english}, où \contenuarg{forme} désigne le type d'élément vers lequel on renvoie.
