...
Pour indexer un document, il faut utiliser le package \package{indextools} et placer  dans le préambule la commande \csp{makeindex}\footnote{Le \package {indextools} remplace à la fois le  \package{makeindex} et le  \package{splitindex}, dont il reprend les commandes. Il est plus simple à utiliser, notamment pour la mise en page de l'index \renvoi{styleindex}, et ne nécessite qu'une compilation, contrairement à ses prédécesseurs. Pour obtenir des informations sur les commandes d'indexation, il faut consulter les manuels de \package{makeindex} et de \package{SplitIndex}.}.
