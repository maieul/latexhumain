...
La numérotation recommence à chaque \cs{beginnumbering}. Il est toutefois possible d'indiquer de recommencer la numérotation  à chaque page \emph{via} la commande \csp{lineation}\verb={page}=.

\begin{attention}
Si vos sections numérotées sont très longues, il peut arriver une erreur de compilation \forme{TeX capacity exceeded}. Il faut alors fractionner vos sections en utilisant de temps en temps la commande \csp{pausenumbering} suivie de \csp{resumenumbering} au sein de  la section numérotée\footnote{Une autre solution serait de régler la configuration de \TeX pour lui allouer plus de mémoire.}.

Cette séquence de commandes correspond \emph{grosso-modo} à la séquence \cs{endnumbering}\cs{beginnumbering} mais sans recommencer la numérotation.
\end{attention}
...
; celui de la commande \cs{linenumincrement} définit la fréquence avec laquelle les lignes seront numérotées. On place ces commandes après \cs{beginnumbering}. 

Ainsi, si l'on veut numéroter toutes les lignes en commençant par la première, il faut mettre :
...
Il est possible d'indiquer que des lignes sont des \enquote{sous-lignes} c'est à dire de stopper temporairement la numérotation et de commencer une sous-numérotation.

On commence la sous-numérotation par \csp{startsub} et on la finit par \csp{endsub}.
...
Les numéros des lignes apparaissent  par défaut dans la marge de gauche. On peut redéfinir cette position par  a commande \cs{linenummargin}\marg{arg}, dont les arguments peuvent être \arg{left} (par défaut), \arg{right}(numéros des lignes à droite), \arg{inner}(numéros dans la marge intérieure), ou \arg{outer}(numéro dans la marge extérieure).

Il existe   cinq styles pour numéroter les lignes, que l'on sélectionne par la commande 
\cs{linenumberstyle}\marg{style} (et, pour les sous-numérotations,\cs{sublinenumberstyle}{\marg{style}) . Le style par défaut est \arg{arabic}: les numéros des lignes sont en chiffres arabes. Les autres styles sont: \arg{Alph} (lettres majuscules), \arg{alph} (lettres minuscules), \arg{Roman} (chiffres romains majuscules) et \arg{roman} (chiffres romains minuscules). 

\cs{linenumsep} représente la distance entre le texte et le numéro de la ligne (par défaut, un pica). Cette distance est donc modifiable par la commande \verb|\setlength{\linenumsep}\marg{distance}. On peut de la même façon modifier \cs{numlabfont}, qui indique la taille du numéro et qui est ainsi définit: \verb|\newcommand{\numlabfont}{\normalfont\scriptsize}|. On peut donc la redéfinir avec \cs{renewcommand}.
...
Ce type de note appelle un \enquote{lemme}, c'est-à-dire une partie du texte annoté qui doit reprise dans la note de bas de page, précédée du numéro de la ou des lignes correspondantes. 
...
La commande \cs{marginpar} ne fonctionne pas dans les textes numérotés. Pour compenser cela, le package propose les commandes \csp{ledleftnote}\marg{note} et \csp{ledrightnote}\marg{note} pour des notes de marge gauche et droite.
...
\package{ledmac} fait correspondre en vis-à-vis chaque \enquote{boîte} d'un texte à la boîte correspondante de l'autre texte. Pour la poésie,  chaque vers  (terminé par \ampersand ), est une boîte;  en prose, les boîtes sont délimitées par  \cs{pstart} et un \cs{pend}.  Il est recommandé, en prose, de mettre chaque paragraphe dans une boîte, pour obtenir une synchronisation la plus fine possible --- donc de débuter chaque paragraphe par \cs{pstart} et de finir par \cs{pend}. 


\package{ledmac} peut mettre les textes sur deux colonnes ou sur deux pages en vis-à-vis : bien sûr dans ce dernier  cas le texte de gauche est toujours imprimé sur une page paire.
On utilise dans le premier cas l'environnement \enviro{pairs} ; l'environnement \enviro{pages} dans le second.

Les commandes \cs{Columns} ou \cs{Pages}, selon qu'on soit dans l'environnement \enviro{pairs} ou \enviro{pages}, indiquent à \LaTeX  d'imprimer les boîtes jusqu'ici gardées en mémoire. 
...
\begin{pages}  % On indique qu'on veut des textes en vis-à-vis
...
Le texte dans une langue 
 ...
 \begin{Rightside}  % Et on recommence la même chose la droite
 \beginnumbering
 \pstart
...
\begin{attention}
Si l'un des textes n'est pas dans la langue principale du document, il faut préciser le changement de langue\renvoi{langue} \emph{à l'intérieur} de l'environnement \enviro{Leftside} ou {Rightside}.
...
On obtient parfois le message d'erreur \enquote{Too many \cs{pstart} without printing}. Pas de panique ! Cela signifie qu'il y a trop de boîtes en mémoire: \LaTeX  doit en imprimer quelques-unes avant de pouvoir continuer.  Il y a deux solutions possible à ce problème.
...
Celle-ci, que l'on peut d'ailleurs coupler avec la première, consiste à  couper son texte en plusieurs grands ensembles de boîtes --- c'est-à-dire en plusieurs environnements \enviro{Leftside} et \enviro{Rightside}, se terminant chacun par \cs{Columns} ou \cs{Pages}.  Cette solution est préférable pour les longs textes. Les divisions internes du texte se reflètent alors dans les environnements \enviro{Leftside} et \enviro{Rightside}.   Mais si l'on met simplement :
...
La commande \cs{memorydump} permet de résoudre ce problème. Elle équivaut en fait à un \cs{endnumbering} immédiatement suivit d'un \cs{beginnumbering}, mais sans réinitialisation. Il faut donc mettre: 
...
Si vous utilisez la commande \cs{numberpstarttrue}\renvoi{numberpstarttrue} pour numéroter les \cs{pstart}s, sachez que la numérotation gauche et la numérotation droite fonctionnent avec deux compteurs différents : \compteur{pstartL} et \compteur{pstartR}.\renvoi{apparencecompteur}\renvoi{manipcompteurs}
\end{plusloins}
...
Si des vers sur des pages parallèles prennent un nombre de lignes inégales, par exemple si le vers de gauche fait deux lignes et celui de droite n'en fait qu'une, alors \package{ledpar} introduit un blanc après le vers le plus long.
...
Par défaut, les colonnes font 45\% d'une largeur normale de texte. On peut modifier ce réglage en redéfinissant les longueurs \csp{Lcolwidth} et \csp{Rcolwidth} respectivement pour la colonne gauche et droite. On utilise pour ce faire la commande \cs{setlength}\renvoi{setlength}\renvoi{longueurrelative}. Par exemple pour avoir 47\% de la largeur normale d'un texte.
...
De même on peut définir un filet de séparation grâce à la longueur \csp{columnrulewidth}\renvoi{unite}.
...
\subsubsection{Numérotation des lignes de droites}

Par défaut, les lignes de droites sont suivies d'un \forme{R}, correspondant au terme \cs{Right}. Pour obtenir à la place un \forme{D}, il suffit de rédéfinir la commande \csp{Rlineflag} :
