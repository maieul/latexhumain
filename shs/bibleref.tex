...
On choisit son style par la commande \cs{biblerefstyle}\marg{style}, qu'il est conseillé de mettre en préambule, ou bien par une option de package :
...
On peut remplacer la commande \cs{bibleverse} par la commande \csp{ibibleverse}. Celle-ci en plus d'afficher une référence biblique l'indexe.

En outre il existe deux commandes, utilisant la même syntaxe que \csp{ibibleverse} : \csp{ibiblechvs}, qui n'affiche pas le nom du livre dans le texte, et \csp{ibiblevs} qui n'affiche dans le texte que le numéro de verset.
