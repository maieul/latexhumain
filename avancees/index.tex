...
Auparavant, il fallait compiler en ligne de commande suivant cette syntaxe :\renvoi{terminal}

\begin{bashcode}
makeindex -s monstyle.ist monfichier.idx
\end{bashcode}

L'option -s indiquait  à \package{makeindex}  qu'il devait utiliser le fichier de style \fichier{monstyle.ist}.
Le package \package{imakeidx}  évite de passerpar le terminal : il suffit en effet de  donner à la commande \cs{makeindex} l'argument \oarg{options=-s  monstyle.ist}
