...
Sans le \package{imakedix}, il fallait compiler en ligne de commande\renvoi{terminal}, selon cette syntaxe :

\begin{bashcode}
makeindex -s monstyle.ist monfichier.idx
\end{bashcode}

Il faut encore procéder ainsi lorsque vous passer l'option \option{noautomatic} à la commande \cs{makeindex} pour un index particulier, comme par exemple pour celui des sources primaires\renvoi{noautomatic}.
...
Si nous préférons qu'elles soient simplement légèrement décalées vers la droite, nous pouvons utiliser, à la place de \textbackslash\cs{hfill}, \textbackslash\cs{hspace*}\verb|{1em}| qui rajoute à gauche un espace d'un cadratin. \renvoi{unite}\renvoi{espace}
