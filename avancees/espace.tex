\chapter{Élèments de mise en page}\label{espacement}
...
\item[ligne 7]une liste ne peut pas fonctionner sans commande \cs{item}. C'est pourquoi elle est appelée ici. Pour éviter des ennuis, on indique que le texte de cet \cs{item} est nul, via la commande \csp{relax} qui est une commande qui sert à ne rien faire alors que \TeX attend un contenu.
\end{description}
...
\begin{plusloins}
Pour des textes encadrés ou sur fond coloré, on pourra utiliser le package \package{mdframed} et~/~ou le package \package{fancybox}\footcites[On peut également consulter][qui regorge d'exemple pratique de \enquote{mise en boîte}]{frama}[on consultera en particulier][]{frama_boites}. 
\end{plusloins}
