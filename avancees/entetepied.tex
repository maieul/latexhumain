...
\LaTeX propose quatre styles standards de pages, définissant le contenu des entête et pieds de page. Pour indiquer à \LaTeX d'utiliser l'un de ces styles, il suffit d'utiliser la commande \commande{pagestyle}
\begin{minted}{latex}
\pagestyle{nom_du_style}
\end{minted} 

On peut également modifier le style d'une page en particulière grâce à la commande \commande{thispagestyle}.
...
\begin{description}
\item[\commande{lhead}]prend comme argument le contenu de la partie gauche de l'entête, justifiée à gauche.
\item[\commande{chead}]prend comme argument la partie centrale de l'entête, centrée.
\item[\commande{rhead}]prend comme argument la partie droite de l'entête, justifiée à droite.
\item[\commande{lfoot}]prend comme argument la partie gauche de l'entête, justifiée à gauche.
\item[\commande{cfoot}]prend comme argument la partie centrale de l'entête, centrée.
\item[\commande{rfoot}]prend comme argument la partie droite de l'entêtre, justifiée à droite.
\end{description}
...
Le principe de base est simple : des commandes de marquage vont stocker en mémoire des marqueurs. Lesdits marqueurs seront appelés par d'autres commandes. Ces commandes sont  deux : \commande{leftmark} et \commande{rightmark}.

Les deux commandes de marquage sont \commande{markboth} et \commande{markright} :
...
\begin{itemize}
\item \commande{leftmark} retourne le premier argument de la dernière commande \commande{markboth}.
\item \commande{rightmark} retourne l'argument \argument{marqueur droit} de la première commande \commande{markright} ou \commande{markboth} \emph{si l'une de ces commandes est présente sur la page en cour}. En revanche, si la page en cours ne contient pas l'une de ces commandes, alors \commande{rightmark} retourne l'argument \argument{marqueur droit} de la dernière commande \commande{markright} ou \commande{markboth} utilisé.
\end{itemize}

Concrètement dans la classe \classe{book}, ces commandes sont appelées par les commandes \commande{chapter} et \commande{section} via les commandes \commande{chaptermark} et \commande{sectionmark}. 

\subsection{Appeler les marqueurs dans les style fancy}

Pour rétablir la présentation originelle, il nous faut insérer les commandes \commande{leftmark} et {rightmark} dans nos commandes \commande{fancyhead}.
...
\subsection{Redéfinir les commandes \commande{chaptermark} et \commande{sectionmark}}

Supposons désormais que vous ne souhaitez voir le titre de section dans les entête. Le plus simple est alors de redéfinir la commande \commande{sectionmark}, pour la rendre nulle :
...
\begin{attention}
Notez bien que nous indiquons que la commande \commande{sectionmark} prend un argument, alors même que nous ne nous en servons pas. Mais lorsque la classe \classe{book} appelle cette commande, elle lui passe bien un argument, qui est le titre de la section.
\end{attention}

Étant donné que vous n'affichez plus que le titre du chapitre, il est peut-être inutile de préciser qu'il s'agit d'un chapitre. Il nous faut alors redéfinir la commande \commande{chaptermark}. Par exemple, sous la forme suivante :
...
\begin{anedocte}
Si vous vous amusez à fouiller le fichier \fichier{book.cls}, vous verrez que la commande \commande{chaptermark} est définie deux fois. En réalité, la définition est conditionné par l'option de classe \option{twoside} ou \option{oneside}.
\end{anedocte}
