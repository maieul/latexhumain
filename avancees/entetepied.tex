...
Supposons que nous souhaitons afficher le numéro de page, en pied de page centré, en indiquant également le nombre total de page. Nous allons utiliser le package \package{lastpage} qui nous permet, après deux compilation\footnote{ou plus si le nombre de page varie entre les compilations} d'obtenir le numéro de la dernière page.
\begin{minted}{latex}
\usepackage{fancyhdr}
\pagestyle{fancy}
\usepackage{lastpage}
...
\begin{attention}
\verb|pageref{LastPage} n'indique pas le nombre total de page, mais bien le numéro de la dernière page. Or certaines commandes comme \commande{mainmatter} ont pour effet de recommencer la numérotation des pages. Toutefois, il est rarement utile d'écrire le nombre total de page sur un travail de type \classe{book} qui est généralement bien relié.
...
\begin{enumerate}
\item[C]centre
\item[LO]gauche des pages recto (= intérieur des pages de droite, si on écrit de gauche à droite).
\item[RO]droite des pages recto (= extérieur des pages de droite, si on écrit de gauche à droite).
\item[LE]gauche des pages verso (= extérieur des pages  de gauche, si on écrit de gauche à droite).
\item[RE]droite des pages verso (= intérieur des pages de gauche, si on écrit de gauche à droite).
\end{enumerate}
...
Lorsqu'on utilise le style de page \argument{fancy}, on supprimer automatiquement les styles par défaut, qui contenait notamment les titres de chapitres.
...
\package{fancyhdr} inclus un filet entre le corps du texte et l'entête. Il est possible de modifier la taille de ce filet ainsi que de rajouter un filet avant le  pied de page.
...
Par exemple, pour indiquer de ne pas mettre de filet en dessous de l'entête mais d'un mettre un de la moitié de la largeur du texte, il faut indiquer :
\begin{minted}{latex}
\renewcommand[\headrulewidth}[0]{0pt}
\renewcommand[\footrulewidth}[0]{0.5\textwidth}
