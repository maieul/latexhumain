...
On peut contourner ce problème en créant des sous-entrées bibliographiques\renvoi{divisionsource}, dont nous parlons dans un autre chapitre. 
\end{attention}

\begin{attention}
Par défaut, cette option ne fonctionne pas lorsque la référence est donnée sous forme abrégé (\emph{op. cit.} ou \emph{ibid.}). Il s'agit d'un bogue des styles standards de \emph{biblatex}, qu'il est aisé de corriger. Nous expliquons comment sur notre blog\footcite{biblio_bugomit}, mais il vous faut lire le deuxième chapitre sur les styles bibliographiques\renvoi{style2} pour comprendre ce que nous y écrivons. 
\end{attention}
