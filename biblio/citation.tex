...
\renewbibmacro*{cite:title}{\usebibmacro{cite:full}}
...
Le champ  \champ{pagination} sert lorsque l'on met une indication numérique dans l'argument \arg{postnote}. En revanche si on laisse le contenu du champ \champ{pages}, il est sans effet. Il faut dans ce cas mettre la même valeur dans le champ \champ{bookpagination}.
...
La commande \csp{DeclareFieldFormat} indique la manière d'afficher le champ \champ{pages} : en passant son contenu (\verb|#1|) à une fonction \csp{mkpageprefix}, à qui on indique d'utiliser le champ \champ{pagination} pour afficher le préfixe de page\footcite[Voir][]{biblatex_formating}.
...
% On rétablis la gestion des ibidems et op. cit
