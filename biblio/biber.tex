...
\inputminted{exemples/biblio/premierpas/reproduction.bib}
...
\inputminted{exemples/biblio/premierpas/crossref.bib}

En toute logique la compilation avec BibTeX devrait attribuer la valeur du champ \champ{title} de l'entrée \verb|title} aux champs \champ{booktitle} des entrées \verb{clef1} et \verb{clef2}, puisqu'il s'agit d'entrée de type \verb|@inproceedings| héritant d'entrée \verb|@proceedings|. 

Il n'en est rien : BibTeX procède à des héritages simples : il attribut les valeurs \enquote{mères} \champ{title} aux champs \champ{title} des fils. Ceci oblige donc à dupliquer les valeurs\footnote{Certain logiciel de gestion bibliographique le font pour vous. Toutefois, cela alourdit le fichier. En outre il s'agit de contourner un limitation technique de BibTeX, plutôt que de remédier à cette limitation.}.
...
Pour la compilation avec Biber fonctionne, il faut indiquer lors du chargement du package qu'on utiliser Biber, via l'option \argument{backend}.
...
Pour compiler avec Biber, il faut bien sûr l'avoir installé\renvoi{installbiber}. Toutefois la plupart des éditeurs \LaTeX ne prévoient pas encore de compiler avec Biber. Il faut passer par la ligne de commande : nous expliquons tout ceci en annexe.

\subsection{Préciser les héritages de champs}

Biber possède un certains nombres de réglages d'héritages de champs. Toutefois on peut vouloir modifier ces héritages, ou en ajouter d'autre. Par exemple les champs \champ{entrysubtype} ne sont, par défauts, pas hérités. 
...
S'il est aisé lorsqu'on utilise une commande de citation d'indiquer une page précise\renvoi{pages}, il n'est pas possible en revanche d'indiquer une division de source précise. La solution est relativement simple : il suffit de créer une entrée principale correspondante à l'ensemble de l'œuvre, et des entrées secondaires correspondantes à des parties de l'œuvre. Ces entrées secondaires hériteront des valeurs de l'entrée principal, sauf pour le champ \champ{pages} et le champ \champ{titleaddon} dans lequel on précisera la division de source.
...
\begin{enumerate}
\item D'un point de vue sémantique, la division de source peut être considéré comme un ajout au titre. Il précise plus précisément de quelle partie de l'œuvre d'un auteur on parle.
\item D'un point de vue pratique, ce champ à pour avantage d'être, par défaut, situé par BibLaTeX entre le titre et les informations d'éditions, tout en étant pas en italique.
...
\inputminted{exemples/biblio/premierpas/augustin.bib}
