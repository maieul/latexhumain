...
\begin{fieldlist}
	\fielditem{afterword} Auteur(s) de la postface. 
   	...
   	\fielditem{editor} Éditeur(s) scientifique(s). On peut en préciser le rôle grâce au champ \champ{editortype}.	
   	\fielditem{editora} Éditeur(s) scientifiques ayant un autre rôle. On peut en préciser le rôle grâce au champ \champ{editoratype}.  
   	\fielditem{editorb} Éditeur(s) scientifiques ayant un autre rôle. On peut en préciser le rôle  grâce au champ \champ{editorbtype}.  
   	\fielditem{editorc} Éditeur(s) scientifiques ayant un autre rôle. On peut en préciser le rôle  grâce au champ \champ{editorctype}\footcite[Pour ces quatres champs, se reporter à][]{biblatex_editortype}.
...
Lorsque des champs possèdent des valeurs identiques, par exemples les champs \champ{editor} et \champ{translator}, \package{biblatex} fusionne ces champs lors de l'affichage. Prenons ainsi l'entrée suivante : 
...
\begin{description}
	...
	\item[Un nom.] L'initiale doit être en majuscule, le reste en minuscules. Exemple : \forme{Londres} et non pas \forme{LONDRES} ; \package{biblatex} se charge le cas échéant de mettre en petites capitales.
	\item[Éventuellement une particule.]Elle doit être entièrement en minuscules. Par exemple : \forme{de}, \forme{von}.
	\item[Éventuellement un suffixe.] L'initiale doit être en majuscule. Ce type de donnée est plutôt anglo-saxonne. Exemple : \forme{Junior}.
\end{description}
...
\begin{fieldlist}
	\fielditem{booksubtitle}Sous-titre du livre dans lequel l'entrée se situe. 
   	...
   	\fielditem{issuesubtitle} Sous-titre d'un numéro spécifique d'un périodique. 	Pour les entrées de type \type{periodical}, le sous-titre du périodique doit aller dans le champ \champ{subtitle}, celui du numéro dans le champ \champ{issuesubtitle}		
   	\fielditem{issuetitle} Titre d'un numéro spécifique d'un périodique. Pour les entrées de type \type{periodical}, le titre du périodique doit aller dans le champ \champ{title}, celui titre du numéro dans le champ \champ{issuetitle}.		
   	\fielditem{journalsubtitle} Sous-titre d'un périodique.							
   	\fielditem{journaltitle} Titre d'un périodique. Le champ \champ{journal} est un alias de ce champ\footnote{Ce qui signifie que remplir le champ \champ{journal} revient à remplir ce champ.}.				
...
\begin{fieldlist}
	...
	Numéro d'édition si plusieurs éditions existent. Doit être un entier ou bien une chaîne de caractères.
   	\fielditem{eventdate} Date du colloque pour les entrées de type \type{proceedings} et \type{inproceedings}. 
	\fielditem{howpublished} Pour les entrées de type \type{misc}, précise le mode de publication.
   	...
   	\fielditem{language} Langue de l'œuvre. Le nom de la langue doit, idéalement, être mis comme indiqué dans la documentation de \package{polyglossia}\footcite{polyglossia}.					
   	...
   	\fielditem{origlanguage} Langue originelle. Le nom de la langue doit, idéalement, être mis comme indiqué dans la documentation de \package{polyglossia}\footcite{polyglossia}. 
	...
	\fielditem{pagetotal} Nombre total de pages.
   	\fielditem{part} Pour les livres en plusieurs volumes \emph{physiques}, indique le numéro du volume physique.  Le numéro du volume \emph{logique} est à indiquer dans le champ \champ{volume}.
   	\fielditem{publisher} Éditeur commercial.					
   	\fielditem{pubstate} Pour les œuvres qui ne sont pas encore imprimées, indique le statut :
					\begin{description}
   	...
   	\fielditem{type} Pour les entrées de type  \type{thesis}, \type{patent} et \type{report}, précise le type de travail.
...
Il peut être utile d'indiquer des informations comme l'ISBN, etc. Voici les champs possibles\footcite[Par défaut, \package{biblatex} imprime ces champs s'ils sont remplis. Il est toutefois possible de ne pas les afficher en passant l'option \option{isbn=false} au chargement du package][]{biblatex_isbn}.  Le lecteur curieux trouvera aisément des informations sur leurs significations.

\begin{fieldlist}
	\fielditem{eid} Identifiant électronique d'une entrée de type \type{article}. 
   	\fielditem{isan} \emph{\textenglish{International Standard Audiovisual Number}}, pour les entrées de type audiovisuel.
   	\fielditem{isbn} \emph{\textenglish{International Standard Book Number}}, pour les livres. 
   	\fielditem{ismn} \emph{\textenglish{International Standard Music Number}}, pour les musiques imprimées, comme par exemple les partitions. 
   	\fielditem{isrn} \emph{\textenglish{International Standard Technical Report Number}}, pour les rapports techniques. 
   	\fielditem{issn} \emph{\textenglish{International Standard Serial Number}}, pour les numéros de revues. 
   	\fielditem{iswc} \emph{\textenglish{International Standard Work Code}} pour les œuvres musicales.
\end{fieldlist}
...
\begin{fieldlist}
	\fielditem{abstract} Résumé de l'œuvre. 
   	...
   	\fielditem{file} Adresse d'une version informatique locale du travail. 
   	\fielditem{library} Annotation sur la disponibilité en bibliothèque, par exemple  le nom de la bibliothèque et la cotation.
