...
\item Le troisième argument contient le code qui sera appelé pour créer l'entête d'une bibliographie. Ici on déclare qu'on passe le titre (le \verb|#1|) à une commande \cs{subsection}.
\end{enumerate}
...
On peut alors passer l'argument \argument{keyword} à la commande \cs{printbibliography}. On peut ainsi sélectionner toutes les entrées ayant la valeur \forme{xxx} dans leurs champs \champ{keywords} :
