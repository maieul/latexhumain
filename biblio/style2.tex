...
Pour ce faire, nous allons modifier les styles de \emph{BibLaTeX}, en utilisant la commande \cs{ifsingletitle{oui}{non}}. Cette commande vérifie si une entrée est la seule attribuée à son auteur, et renvoie \argument{oui} si c'est le cas, \argument{non} sinon. 
...
\item[Ligne 5]la commande \cs{bibstring} sert à appeler une chaîne de langue, ici \verb|opcit|. Le premier argument \argument{\cs{mkibid}} sert à indiquer que la chaîne de langue est passée à la commande \cs{mibid} avant d'être affichée.
\end{description}
